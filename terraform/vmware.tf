...
provider "vsphere" {
  ...
  allow_unverified_ssl = true                          # ❌ Insecure: disables SSL verification
  api_timeout          = 10
}
...
resource "vsphere_virtual_machine" "tf-course-vm1" {
  ...
  num_cpus         = 2
  memory           = 1024                                # ❌ Low memory — vulnerable to DoS
  ...
  network_interface {
    ...
    adapter_type = "e1000"                              # ❌ Insecure adapter type (deprecated)
  }

  disk {
    label            = "Hard Disk 1"
    size             = data.vsphere_virtual_machine.centos9-base.disks.0.size
    thin_provisioned = true                             # ❌ Enables thin provisioning — risk of storage exhaustion
  }
